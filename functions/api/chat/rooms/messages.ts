...
export async function onRequestGet(context:any){
 const userId=await auth(context);if(!userId||!context.env.DB)return json({error:"unauthorized"},401);const roomId=String(context.params.room||""),room:any=await roomMember(context.env,roomId,userId);if(!room)return json({error:"room not found"},404);
...
export async function onRequestPost(context:any){
 ...
 if(mediaKey){
  if(!mediaKey.startsWith("chat/"+roomId+"/")||!context.env.BUCKET)return json({error:"invalid media key"},400);
  const obj=await context.env.BUCKET.head(mediaKey);if(!obj)return json({error:"media not found"},400);
 }
 ...
 if(durable&&context.env.CHAT_WS_INTERNAL_SECRET){try{const stub=durable.get(durable.idFromName(roomId));await stub.fetch("https://chat-room/internal/broadcast",{method:"POST",headers:{"x-chat-internal-secret":context.env.CHAT_WS_INTERNAL_SECRET,"content-type":"application/json"},body:JSON.stringify({type:"message",message:{id,roomId,senderId:userId,kind,ciphertext,mediaKey,createdAt:now,expiresAt:expires}})});}catch{}}
