...
export async function onRequestPost(context:any){
 ...
 const roomId=String(context.params.room||""),room:any=await context.env.DB.prepare("SELECT r.id,r.expires_at FROM rooms r JOIN room_members m ON m.room_id=r.id WHERE r.id=? AND m.user_id=? AND r.expires_at>?").bind(roomId,userId,Date.now()).first();
 ...
 const id=crypto.randomUUID(),key="chat/"+roomId+"/"+id+".bin";
