...
export async function onRequestGet(context:any){
 const c=context.request.headers.get("Cookie")||"";const t=c.match(/(?:^|;\\s*)chat_session=([^;]+)/)?.[1];const uid=t&&context.env.CHAT_SESSIONS?await context.env.CHAT_SESSIONS.get(t,"json"):null;if(!uid)return json({error:"unauthorized"},401);
