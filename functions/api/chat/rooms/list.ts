...
export async function onRequestGet(context:any){
 ...
 const now=Date.now();const rows=await context.env.DB.prepare("SELECT r.id,r.name,r.expires_at,r.created_at FROM rooms r JOIN room_members m ON m.room_id=r.id WHERE m.user_id=? AND r.expires_at>? ORDER BY r.created_at DESC").bind(uid,now).all();
