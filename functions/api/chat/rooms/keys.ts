...
export async function onRequestPost(context:any){
 ...
 const member=await context.env.DB.prepare("SELECT 1 FROM room_members m JOIN rooms r ON r.id=m.room_id WHERE m.room_id=? AND m.user_id=? AND r.expires_at>?").bind(roomId,uid,Date.now()).first();
 const device=await context.env.DB.prepare("SELECT 1 FROM devices WHERE id=? AND user_id=?").bind(deviceId,uid).first();
 if(!member||!device)return json({error:"forbidden"},403);
...
export async function onRequestGet(context:any){
 const uid=await auth(context);if(!uid||!context.env.DB)return json({error:"unauthorized"},401);
 const roomId=String(context.params.room||""),member=await context.env.DB.prepare("SELECT 1 FROM room_members m JOIN rooms r ON r.id=m.room_id WHERE m.room_id=? AND m.user_id=? AND r.expires_at>?").bind(roomId,uid,Date.now()).first();
 if(!member)return json({error:"forbidden"},403);
 const r=await context.env.DB.prepare("SELECT k.device_id,k.encrypted_key,d.public_key FROM room_keys k JOIN devices d ON d.id=k.device_id WHERE k.room_id=? AND d.user_id=?").bind(roomId,uid).all();
 return json({keys:r.results||[]});
}
